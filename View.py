...
class Toplevel1:
    ...
    def setup_player(self):
        ...
        img = tk.PhotoImage(file="g:/coremusicplayergui/icons/music1.png")
    ...
    def add_song_to_favourites(self):
        afv_song_index_tuple = self.platList.curselection()
        try:
            if len(fav_song_index_tuple) == 0:
                raise noSongSelectederror("Plese select a song to add to favourites")
            song_name = self.playList.get(fav_song_index_tuple[0])
            result = self.my.player.add_song_to_favourites(song_name)
    ...
    def load_song_from_favourites(self):
        try:
            load_result = self.my_palyer.load_song_from_favourties()
            result = load_result[0]
            if result.find("No song present") != -1:
                messagebox.showinfo("favourites empty!!!,"
                No
                songs in your
                favourites
                ")
            ...
            self.addSongsToPlayList.delete(0, tk.END)
            for song_name in song_dict:
                self.playList.insert(tk.END, song_name)
                print("from db:", song_name)
            rcolr = lambda: random.randint(0, 255)
            red = hex(rcolor())
            green = hex(rcolor())
            blue = hex(rcolor())
            ...
            print(mycolor)
            self.playList.configure(fg=mycolor)
            messagebox.showinfo("Success!!!", "list popuated from your fav!!!")
    ...
    def remove_song_from_favourites(self):
        fav_song_index_tuple = playList.curselection()
        try:
            if len(fav_song_index_touple) == 0:
